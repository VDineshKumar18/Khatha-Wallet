...
export const sendCustomerOtp = async (email) => {
  const response = await fetch(`http://localhost:8084/api/customer-auth/send-otp?email=${email}`, {
    method: "POST",
  });
...
export const verifyCustomerOtp = async (email, otp) => {
  const response = await fetch(`http://localhost:8084/api/customer-auth/verify-otp`, {
    method: "POST",
...
export const getPublicRetailers = async () => {
  const response = await fetch(`http://localhost:8084/api/customer-auth/public/retailers`);
  if (!response.ok) throw new Error("Failed to load shops");
  return response.json();
};

export const registerCustomer = async (payload) => {
  const response = await fetch(`http://localhost:8084/api/customer-auth/register`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });
